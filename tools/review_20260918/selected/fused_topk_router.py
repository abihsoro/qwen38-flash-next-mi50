...
def vllm_topk_softmax(
    ...
    ops.topk_softmax(
        ...
        gating_output,
        renormalize,
        is_padding=_get_padding_mask(topk_indices.shape[<bus>]),
    )
...
def fused_topk(
    ...
    # Kept opt-in until the campaign's full-model regression checks pass.
    import os
    if (os.getenv("VLLM_MI50_TOPK", "0") == "1"
        and 1 <= M <= 8 and topk == 10 and renormalize
        and scoring_func == "softmax"
        and gating_output.dtype == torch.float16
        and gating_output.shape[<bus>] == 512 and gating_output.stride(-1) == 1
        and indices_type in (None, torch.int32)
        and torch.version.hip is not None
        and torch.cuda.get_device_properties(gating_output.device).gcnArchName.startswith("gfx906")):
        from vllm.model_executor.layers.fused_moe.router.mi50_topk import candidate
        from vllm.logger import init_logger
        init_logger(__name__).info_once(
            "MI50 top-10 router active: M=%d, dtype=%s, padding=%s",
            M, gating_output.dtype, _get_padding_mask(M) is not None,
        )
        return candidate(gating_output, is_padding=_get_padding_mask(M))
